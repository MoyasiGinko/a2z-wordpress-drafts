...
.a2zh-path-media {
  ...
  border-radius: 16px;
  border: 1px solid rgba(26, 111, 168, 0.16);
  box-shadow: 0 16px 42px rgba(13, 79, 124, 0.15);
}
    ...
    <div class="a2zh-wrap">
      ...
      <div class="a2zh-path" id="a2zhPath">
        <svg class="a2zh-path-svg" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
          <polyline class="a2zh-path-base" points=""></polyline>
          <polyline class="a2zh-path-prog" points=""></polyline>
        </svg>
        <?php foreach ($switch_steps as $i => $st) :
          $side = ($i % 2 === 0) ? 'left' : 'right';
        ?>
        <div class="a2zh-path-item a2zh-path-item--<?php echo $side; ?> a2zh-rv">
          <figure class="a2zh-path-media">
...
(function() {
  ...
  if (path) {
    ...
    var buildPath = function() {
      ...
      // connect point = the middle centre of each image
      var pts = pathItems.map(function(it) {
        var media = it.querySelector(".a2zh-path-media");
        var mr = media.getBoundingClientRect();
        var x = mr.left - pr.left + mr.width / 2;
        var y = mr.top - pr.top + mr.height / 2;
        return [x, y];
      });
      var str = pts.map(function(p) {
        return p[0].toFixed(1) + "," + p[1].toFixed(1);
      }).join(" ");
      baseLine.setAttribute("points", str);
      progLine.setAttribute("points", str);
    ...
    if ("IntersectionObserver" in window) {
      var focusIO = new IntersectionObserver(function(entries) {
        entries.forEach(function(en) {
          var idx = pathItems.indexOf(en.target);
          en.target.classList.toggle("is-focus", en.isIntersecting);
          if (dots[idx]) dots[idx].classList.toggle("is-on", en.isIntersecting);
        });
      }, {
        threshold: 0,
      ...
      pathItems.forEach(function(it, i) {
