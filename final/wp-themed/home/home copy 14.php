...
/* ════ THREE PODS — number on top, thread-connected, auto-highlight ════ */
.a2zh-pods3 {
  position: relative;
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
...
/* bold thread linking the three number badges across the top */
.a2zh-pods3-thread {
  position: absolute;
  top: 27px;
  left: 0;
...
.a2zh-pod3-num {
  ...
  box-shadow: 0 6px 18px rgba(13, 79, 124, 0.12);
  position: relative;
  z-index: 2;
  margin-bottom: 18px;
...
/* highlighted state (auto-cycled, or held while hovered) */
.a2zh-pod3.is-active .a2zh-pod3-num {
  background: linear-gradient(135deg, var(--blue), var(--blue-dk));
  color: #fff;
  border-color: var(--blue);
  transform: scale(1.12);
  box-shadow: 0 10px 26px rgba(26, 111, 168, 0.4);
}

.a2zh-pod3.is-active .a2zh-card {
  border-color: rgba(45, 147, 212, 0.55);
  box-shadow: 0 26px 56px rgba(8, 29, 51, 0.2), 0 0 0 1px rgba(45, 147, 212, 0.2);
...
(function() {
  ...
  if (pods3) {
    ...
    var cyclePod3 = function() {
      ...
      var idx;
      do {
        idx = Math.floor(Math.random() * pod3Items.length);
      } while (idx === pod3Last);
      pod3Last = idx;
      setPod3(idx);
    };
