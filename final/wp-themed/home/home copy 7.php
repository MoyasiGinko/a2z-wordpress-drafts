...
<main class="a2zh-home" id="a2zh-home">
  ...
  <section class="a2zh-section">
    <div class="a2zh-wrap">
      ...
      <div class="a2zh-grid-3">
        ...
        ?>
        <article class="a2zh-card a2zh-feature a2zh-rv a2zh-d<?php echo $i; ?>">
          <?php if ($icon) : ?>
          <img class="a2zh-feature-icon" src="<?php echo esc_url($icon['url']); ?>"
            alt="<?php echo esc_attr($icon['alt'] ?: get_sub_field('title')); ?>" />
          <?php else : ?>
          <span class="a2zh-feature-num"><?php echo sprintf('%02d', $i); ?></span>
          <?php endif; ?>
        ...
        ?>
        <article class="a2zh-card a2zh-feature a2zh-rv a2zh-d<?php echo $i + 1; ?>">
          <span class="a2zh-feature-num"><?php echo sprintf('%02d', $i + 1); ?></span>
  ...
  <section class="a2zh-section a2zh-section--tint">
    <div class="a2zh-wrap">
      <div class="a2zh-split a2zh-split--rev">
        <figure class="a2zh-media a2zh-rv">
          <img
        ...
        <div>
          <span class="a2zh-label a2zh-rv">Who this is for</span>
          <h2 class="a2zh-title a2zh-rv a2zh-d1">
            <?php echo get_field('why_choose_title') ?: $who_title_default; ?>
          </h2>
          <?php if (get_field('why_choose_content')) : ?>
          <div class="a2zh-copy a2zh-copy-spaced a2zh-rv a2zh-d2"><?php echo get_field('why_choose_content'); ?></div>
          <?php endif; ?>
          <div class="a2zh-list a2zh-rv a2zh-d2">
            <?php foreach ($who_items as $it) : ?>
            <div class="a2zh-list-item"><strong><?php echo $it[0]; ?></strong><span><?php echo $it[1]; ?></span></div>
            <?php endforeach; ?>
          </div>
  ...
  <section class="a2zh-section">
    <div class="a2zh-wrap">
      <div class="a2zh-split">
        <div>
          <span class="a2zh-label a2zh-rv">Our responsibility</span>
          <h2 class="a2zh-title a2zh-rv a2zh-d1">
            <?php echo get_field('our_promise') ?: $promise_default_title; ?>
          </h2>
          <?php if (get_field('our_promise_content')) : ?>
          <div class="a2zh-copy a2zh-copy-spaced a2zh-rv a2zh-d2"><?php echo get_field('our_promise_content'); ?></div>
          <?php else : ?>
          <p class="a2zh-copy a2zh-copy-spaced a2zh-rv a2zh-d2">Reactive accounting creates unnecessary stress and
            missed opportunities. Established businesses deserve ongoing, structured oversight, not a call once a year
            when it is too late to change anything.</p>
          <?php endif; ?>
          <ul class="a2zh-plist a2zh-rv a2zh-d2">
            <?php foreach ($promise_points as $pp) : ?>
            <li><?php echo $pp; ?></li>
            <?php endforeach; ?>
          </ul>
          <blockquote class="a2zh-quote a2zh-rv a2zh-d3"><?php echo $promise_quote; ?></blockquote>
          <div class="a2zh-actions a2zh-rv">
        ...
        <figure class="a2zh-media a2zh-rv a2zh-d1">
          <img
    ...
    <div class="a2zh-wrap">
      ...
      <div class="a2zh-sectors">
        ...
        <a class="a2zh-card a2zh-sector a2zh-rv" href="<?php echo esc_url($link_url); ?>">
          <h3><?php echo get_sub_field('title'); ?></h3>
          <p><?php echo get_sub_field('content'); ?></p>
          <span class="a2zh-sector-link">Explore &rarr;</span>
        </a>
        <?php
          endwhile;
        else :
  ...
  <section class="a2zh-section">
    <div class="a2zh-wrap">
      <div class="a2zh-journey-head a2zh-rv">
        <h2 class="a2zh-title">
          <?php echo get_field('switching') ?: 'Structured, straightforward <span>from day one.</span>'; ?>
        </h2>
        <p class="a2zh-copy a2zh-copy-spaced">
          <?php echo get_field('switching_content') ?: 'We coordinate directly with your previous accountant so the transition is clean, complete and without disruption to your business.'; ?>
        </p>
      </div>
      ...
      <div class="a2zh-actions a2zh-rv" style="justify-content:center;margin-top:clamp(36px,5vw,56px);">
...
(function() {
  ...
  if (path) {
    ...
    var roundCurve = function(a, b, bulge, side) {
      ...
      if (nx < 0) { nx = -nx; ny = -ny; }   // normalise perp to point right
      nx *= side; ny *= side;               // then flip to the chosen side
    ...
    var buildPath = function() {
      ...
      pathItems.forEach(function(it) {
        ...
        var L = mr.left - pr.left, R = mr.right - pr.left,
          T = mr.top - pr.top, B = mr.bottom - pr.top;
        var cls = media.getAttribute("class") || "";
        var idxs = [];
        if (cls.indexOf("bite-br") > -1) { idxs.push(pts.length); pts.push([R, B]); }
        if (cls.indexOf("bite-l") > -1) {
          idxs.push(pts.length); pts.push([L, T]);
          idxs.push(pts.length); pts.push([L, B]);
        }
        if (cls.indexOf("bite-tr") > -1) { idxs.push(pts.length); pts.push([R, T]); }
        if (!idxs.length) { idxs.push(pts.length); pts.push([L + mr.width / 2, T + mr.height / 2]); }
      ...
      var d = "";
      if (pts.length) {
        // build flowing waypoints: every dot, plus one far-out bulge point
        // half-way along each segment (alternating sides), then run a single
        // smooth Catmull-Rom spline through them all — a continuous
        // serpentine wave that weaves through the dots rather than
        // doubling back at them.
        var wp = [pts[0]];
        for (var i = 0; i < pts.length - 1; i++) {
          var a = pts[i], b = pts[i + 1];
          var side = (i % 2 === 0) ? 1 : -1;
          var room = side > 0 ? (pr.width - Math.max(a[0], b[0])) : Math.min(a[0], b[0]);
          var bulge = room * (0.4 + rand(i + 1) * 0.16);
          var dx = b[0] - a[0], dy = b[1] - a[1];
          var len = Math.sqrt(dx * dx + dy * dy) || 1;
          var nx = -dy / len, ny = dx / len;
          if (nx < 0) { nx = -nx; ny = -ny; }
          nx *= side; ny *= side;
          wp.push([(a[0] + b[0]) / 2 + nx * bulge, (a[1] + b[1]) / 2 + ny * bulge]);
          wp.push(b);
        }
        d = "M" + wp[0][0].toFixed(1) + "," + wp[0][1].toFixed(1);
        for (var j = 0; j < wp.length - 1; j++) {
          var q0 = wp[j - 1] || wp[j], q1 = wp[j], q2 = wp[j + 1], q3 = wp[j + 2] || q2;
      ...
      // re-apply focus highlight to each focused item's dot(s)
      pathItems.forEach(function(it, ii) {
        if (it.classList.contains("is-focus") && itemDotIdx[ii]) {
          itemDotIdx[ii].forEach(function(di) {
            if (dots[di]) dots[di].classList.add("is-on");
          });
        }
      });
      updateProgress();
    ...
    if ("IntersectionObserver" in window) {
      var focusIO = new IntersectionObserver(function(entries) {
        entries.forEach(function(en) {
          var ii = pathItems.indexOf(en.target);
          en.target.classList.toggle("is-focus", en.isIntersecting);
          if (itemDotIdx[ii]) {
            itemDotIdx[ii].forEach(function(di) {
              if (dots[di]) dots[di].classList.toggle("is-on", en.isIntersecting);
            });
          }
        });
      }, {
