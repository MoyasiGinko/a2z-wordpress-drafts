...
@media (max-width: 900px) {
  ...
  .a2zh-hub-media {
    order: -1;
    width: min(72%, 260px);
    margin: 0 auto clamp(10px, 3vw, 22px);
  }
  ...
  <section class="a2zh-section a2zh-promise">
    <div class="a2zh-wrap">
      <div class="a2zh-promise-head a2zh-rv">
        <span class="a2zh-promise-eyebrow">Our responsibility</span>
        ...
        <?php else : ?>
        <p class="a2zh-promise-intro">Reactive accounting creates unnecessary stress and missed opportunities.
          Established businesses deserve ongoing, structured oversight, not a call once a year when it is too late to
          change anything.</p>
        <?php endif; ?>
      </div>
  ...
  if (hub) {
    ...
    var buildHub = function() {
      ...
      hubPts.forEach(function(pt) {
        ...
        var c1x = sx + (nx - sx) * 0.5, c1y = sy; // gentle horizontal ease
        var c2x = nx - (nx - sx) * 0.5, c2y = ny;
        s += '<path class="a2zh-hub-line" d="M' + sx.toFixed(1) + ',' + sy.toFixed(1) +
          ' C' + c1x.toFixed(1) + ',' + c1y.toFixed(1) + ' ' +
          c2x.toFixed(1) + ',' + c2y.toFixed(1) + ' ' +
          nx.toFixed(1) + ',' + ny.toFixed(1) + '"></path>';
        s += '<circle class="a2zh-hub-end" cx="' + nx.toFixed(1) + '" cy="' + ny.toFixed(1) + '" r="4"></circle>';
      });
      hubSvg.innerHTML = s;
    };
