...
// Global Output Buffer: Intercepts the final rendered HTML output (including WP Rocket / theme headers)
// and converts 100% of <link rel="stylesheet"> tags into non-blocking async preloads
ob_start(function($buffer) {
    if (is_admin() || empty($buffer)) return $buffer;
    
    return preg_replace_callback('/<link\s+([^>]*rel=[\'"]stylesheet[\'"][^>]*)>/i', function($matches) {
        $tag = $matches[0];
        $attr = $matches[1];
        
        // Skip if already has onload or preload
        if (strpos($attr, 'onload=') !== false || strpos($attr, 'preload') !== false) {
            return $tag;
        }
        
        if (preg_match('/href=[\'"]([^\'"]+)[\'"]/i', $attr, $href_match)) {
            $href = $href_match[1];
            return '<link rel="preload" as="style" href="' . esc_url($href) . '" onload="this.onload=null;this.rel=\'stylesheet\'"><noscript>' . $tag . '</noscript>';
        }
        return $tag;
    }, $buffer);
});
...
add_action('wp_head', function() {
    // Preload the hero background image so browser fetches it immediately
    // bypassing WP Rocket's lazy-background loading for the LCP element
    echo '<link rel="preload" as="image" href="https://www.communitytax.com/wp-content/uploads/2023/09/File-or-Pay-Taxes.webp" fetchpriority="high">';
    // Preload the first hero testimonial video thumbnail as it is the first visible card
    echo '<link rel="preload" as="image" href="https://a2zaccounting.co.uk/wp-content/uploads/2025/06/Testimonial-%E2%80%93-a2z-accounting-06-04-2025_12_21_PM-21.png" fetchpriority="high">';
    // Inline critical hero background CSS & header CLS fixes so WP Rocket cannot lazy-load or shift layout
    echo '<style>
        .hero { background-image: linear-gradient(rgba(0,0,0,.705),rgba(0,0,0,.705)),url("https://www.communitytax.com/wp-content/uploads/2023/09/File-or-Pay-Taxes.webp")!important; background-size: cover!important; background-position: center!important; }
        .leftrating { min-height: 24px!important; contain: size layout!important; }
        .leftrating img, img[alt="rating-stars"], .a2zh-rating-stars { width: 90px!important; height: 18px!important; aspect-ratio: 90/18!important; display: inline-block!important; }
    </style>';
});

// Convert all enqueued theme/plugin CSS stylesheets to non-blocking async preloads (eliminates 2,870ms render-blocking delay)
add_filter('style_loader_tag', function($html, $handle, $href, $media) {
    ...
    if (strpos($html, 'rel=\'stylesheet\'') !== false || strpos($html, 'rel="stylesheet"') !== false) {
        ...
        $async_html = str_replace('rel="stylesheet"', 'rel="preload" as="style" onload="this.onload=null;this.rel=\'stylesheet\'"', $async_html);
        return $async_html . "<noscript>" . $html . "</noscript>";
    }
    return $html;
}, 999, 4);

// Defer non-critical enqueued JS scripts so JavaScript execution never blocks HTML parsing
add_filter('script_loader_tag', function($tag, $handle, $src) {
    if (is_admin()) return $tag;
    if (strpos($tag, 'defer') === false && strpos($tag, 'async') === false && strpos($handle, 'jquery-core') === false) {
...
<?php /* Preconnects for fonts.googleapis.com and fonts.gstatic.com are already
   output by the WordPress theme header — adding them again here creates
   duplicate 'Unused preconnect' warnings in Lighthouse which hurt the
   Network Dependency Tree score. Removed to keep preconnect list clean. */ ?>
<link rel="preload" href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900&family=Libre+Caslon+Text:ital,wght@0,400;0,700;1,400&family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Hanken+Grotesk:wght@400;500;600;700;800&family=Manrope:wght@400;500;600;700;800&family=Poltawski+Nowy:ital,wght@0,600;1,600&family=Cormorant+Garamond:ital,wght@0,500;0,600;0,700;1,500;1,600;1,700&family=DM+Sans:wght@400;500;700&display=optional" as="style" onload="this.onload=null;this.rel='stylesheet'">
<noscript><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;0,800;0,900;1,400;1,500;1,600;1,700;1,800;1,900&family=Libre+Caslon+Text:ital,wght@0,400;0,700;1,400&family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Hanken+Grotesk:wght@400;500;600;700;800&family=Manrope:wght@400;500;600;700;800&family=Poltawski+Nowy:ital,wght@0,600;1,600&family=Cormorant+Garamond:ital,wght@0,500;0,600;0,700;1,500;1,600;1,700&family=DM+Sans:wght@400;500;700&display=optional"></noscript>
...
<style>
        ...
        :root {
            ...
            /* Transitions */
            --transition-smooth: transform 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94), opacity 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94), background-color 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94), border-color 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94), box-shadow 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94), color 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94);
        ...
        *, *::before, *::after {
        ...
        .a2z-new-home-wrapper {
            ...
            /* CLS fix: isolate this wrapper's layout so font/widget shifts
               inside don't bubble up to cause page-level layout shifts */
            isolation: isolate;
        }
	...
	.btn::before {
        ...
        .hero-badge-pill {
            ...
            /* CLS fix: reserve height ahead of font load so badge doesn't shift */
            min-height: 42px;
            contain: layout;
        }
        ...
        @keyframes ring-spin {
            0% { stroke-dasharray: 63; stroke-dashoffset: 63; stroke: #E8D595; }
            50% { stroke-dashoffset: 0; stroke: #D4A84F; }
            100% { stroke-dashoffset: -63; stroke: #E8D595; }
        }
        ...
        .tax-dot-target::after, .tax-dot-hollow::after {
            content: '';
            position: absolute;
            top: -4.5px;
            left: -4.5px;
        ...
        .pg-icon svg path {
            animation: pg-icon-draw 4s ease-in-out 2;
        ...
        /* Three Things Section (A2Z3T) styles from A2Z_Three_Things_FINAL.html */
        .a2z3t {
            background: #FFFFFF;
            padding: 60px 16px;
        ...
        .a2z3t-card {
            position: relative;
            border-radius: 20px;
            border: 1px solid rgba(196,144,90,.7);
            background-image: radial-gradient(120% 70% at 20% 0%,rgba(255,255,255,.75),rgba(255,255,255,0) 55%),repeating-linear-gradient(45deg,rgba(196,144,90,.035) 0 1px,rgba(0,0,0,0) 1px 7px),linear-gradient(170deg,#FDFBF7,#F3EBDC);
            box-shadow: 0 28px 54px -28px rgba(13,43,66,.5);
        ...
        .a2z3t-card::after {
            ...
            position: absolute;
            inset: 9px;
            border: 1px solid rgba(196,144,90,.45);
            border-radius: 13px;
            pointer-events: none;
        }
        .a2z3t-ghost {
        ...
        .a2z3t-seal {
            ...
            height: 66px;
            border-radius: 50%;
            background: linear-gradient(150deg,#16456b,#0D2B42 55%,#0a2236);
            border: 2px solid #C4905A;
            box-shadow: 0 0 0 4px #FFFFFF,inset 0 0 0 3px rgba(216,175,130,.4),0 10px 22px -8px rgba(13,43,66,.6);
            display: flex;
            align-items: center;
            justify-content: center;
            z-index: 2;
        ...
        @media (hover:hover) and (min-width:900px) {
            ...
            .a2z3t-card:hover {
                ...
                box-shadow: 0 36px 64px -30px rgba(13,43,66,.6);
            }
        }

        /* Three Levels Section (A2ZLV) styles from A2Z_Three_Levels_IMPROVED.html */
        .a2zlv {
            position: relative;
            background: #FFFFFF;
        ...
        .a2zlv-card {
            position: relative;
            border-radius: 20px;
            background: linear-gradient(165deg,#123a5c 0%,#0D2B42 45%,#0a2236 100%);
            padding: 26px 24px 28px;
            border: 1px solid rgba(196,144,90,.35);
            box-shadow: 0 24px 46px -26px rgba(13,43,66,.55);
        }
        .a2zlv-card::before {
            content: "";
            position: absolute;
        ...
        .a2zlv-card.t2 {
            border-color: rgba(196,144,90,.6);
        }
        .a2zlv-card.t3 {
            border: 1.5px solid #C4905A;
            box-shadow: 0 0 0 5px rgba(196,144,90,.14),0 26px 50px -26px rgba(13,43,66,.65);
        }
        .a2zlv-tile {
            display: flex;
            width: 74px;
            height: 74px;
            border-radius: 17px;
            background: rgba(216,175,130,.1);
            border: 1px solid rgba(216,175,130,.5);
            box-shadow: inset 0 1px 0 rgba(255,255,255,.12);
            align-items: center;
            justify-content: center;
            margin-bottom: 16px;
        }
        .a2zlv-card.t3 .a2zlv-tile {
            background: linear-gradient(150deg,#C4905A,#D8AF82);
            border-color: #D8AF82;
        ...
        .a2zlv-badge {
            ...
            align-items: center;
            justify-content: center;
            background: linear-gradient(90deg,#B07D3C,#D8AF82);
            color: #0D2B42;
            font-size: 9.5px;
            letter-spacing: .13em;
            font-weight: 800;
...
<!-- ==========================================
    ...
    <section class="hero">
        <div class="container hero-content">
            ...
            <div class="hero-actions">
                <a href="<?php echo esc_url(get_permalink()); ?>#a2z-enquire" class="btn btn-solid">Start Your Enquiry</a>
                <a href="<?php echo esc_url(get_permalink()); ?>#a2z-feequote" class="btn btn-outline">Get A Fee Estimate</a>
            </div>
            ...
            <div class="hero-checklist">
                <div class="hero-checklist-item">
                    ...
                    <span style="text-align: left; display: block;">Tax Planning Included Free</span>
                </div>
                <div class="hero-checklist-item">
                    <div class="checklist-icon-circle">
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="hero-card-container">
                <!-- Decorative gold corner accent (presentation only) -->
                <div class="hero-card-offset-border" aria-hidden="true"></div>
                <div class="hero-card" id="heroTestimonialCard">
                    <!-- Left Video Panel -->
                    <div class="hero-card-video">
                        <img
                            id="heroVideoThumb"
                            src="https://a2zaccounting.co.uk/wp-content/uploads/2025/06/Testimonial-%E2%80%93-a2z-accounting-06-04-2025_12_21_PM-21.png"
                            alt="Testimonial speaking in video"
                        ...
                        <div id="heroVideoPlay" class="hero-card-play" onclick="playLoomVideo()">
                            <img src="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/icons/play-fill.svg" alt="Play Video" width="24" height="24" style="width: 24px; height: 24px;">
                        </div>
                    </div>
    ...
    <!-- ==========================================
       ...
       ========================================== -->
    <section class="a2z3t" id="services">
        <div class="a2z3t-inner">
            <h2>Three Things That Change When You <em>Work With A2Z.</em></h2>
            <div class="a2z3t-grid">
                <!-- Card 1 -->
                <div class="a2z3t-card">
                    <span class="a2z3t-ghost" aria-hidden="true">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#C4905A" stroke-width="1" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="3" y="5" width="18" height="16" rx="2.5"/>
                            <path d="M3 9.5h18M8 2.5V6M16 2.5V6"/>
                            <path d="M9.2 15.2l2.1 2.1 3.9-4.2"/>
                        </svg>
                    </span>
                    <span class="a2z3t-seal">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#D8AF82" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                            <rect x="3" y="5" width="18" height="16" rx="2.5"/>
                            <path d="M3 9.5h18M8 2.5V6M16 2.5V6"/>
                            <path d="M9.2 15.2l2.1 2.1 3.9-4.2" stroke-width="1.8"/>
                        </svg>
                    </span>
                    <div class="a2z3t-body">
                        <h3>Structured Tax Planning</h3>
                        <p>We plan your tax months ahead, not in a year-end rush. You keep more, with no surprises.</p>
                    </div>
                </div>
                ...
                <div class="a2z3t-card">
                    ...
                    <span class="a2z3t-seal">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#D8AF82" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12.5" r="9"/>
                            <path d="M12 12.5V6.8" stroke-width="1.9"/>
                            <path d="M12 12.5l3.6 2.1"/>
                            <path d="M12 1.5v1.6"/>
                        </svg>
                    </span>
                    <div class="a2z3t-body">
                        ...
                        <p>Ask before midday and get an answer the same day, guaranteed in writing. Your business never waits.</p>
                    </div>
                </div>
                ...
                <div class="a2z3t-card">
                    ...
                    <span class="a2z3t-seal">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#D8AF82" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M12 2l8 3.5v5.5c0 5-3.4 9.3-8 10.5-4.6-1.2-8-5.5-8-10.5V5.5z"/>
                            <path d="M8.6 12.2l2.3 2.3 4.5-4.8" stroke-width="1.8"/>
                        </svg>
                    </span>
                    <div class="a2z3t-body">
                        <h3>Complete Financial Confidence</h3>
                        <p>Zero missed deadlines and zero HMRC penalties since we started. Every figure is checked before it reaches you.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- ==========================================
       ...
       ========================================== -->
    <section class="a2zlv" id="levels">
        <div class="a2zlv-inner">
            <h2>Three Levels. <em>One Standard.</em></h2>
            <div class="a2zlv-grid">
                <!-- Card 1 -->
                <div class="a2zlv-card t1">
                    <span class="a2zlv-tile">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#D8AF82" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M12 21.5v-8"/>
                            <path d="M12 13.5C7.5 13.5 5 11 4.8 6.8 9 7 11.8 9.3 12 13.5z" fill="rgba(216,175,130,.25)"/>
                            <path d="M12 11.5c.2-3.6 2.7-5.6 6.6-5.8-.2 3.8-2.6 5.8-6.6 5.8z" fill="rgba(216,175,130,.25)"/>
                        </svg>
                    </span>
                    <span class="a2zlv-kick">Small Start-Up or Micro</span>
                    <div class="a2zlv-name">
                        <h3>Foundational</h3>
                    </div>
                    <p>Everything you must get right: year end accounts, corporation tax, confirmation statement and HMRC deadlines, all done flawlessly and on time.</p>
                </div>

                <!-- Card 2 -->
                <div class="a2zlv-card t2">
                    <span class="a2zlv-tile">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#D8AF82" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M4 20.5h16.5"/>
                            <path d="M4.5 16l4.5-4.5 3.2 3L19 7.5"/>
                            <path d="M15.2 7.5H19V11.3"/>
                        </svg>
                    </span>
                    <span class="a2zlv-kick">Growing, VAT-Registered</span>
                    <div class="a2zlv-name">
                        <h3>Structured</h3>
                    </div>
                ...
                <!-- Card 3 -->
                <div class="a2zlv-card t3">
                    <span class="a2zlv-tile">
                        <svg viewBox="0 0 24 24" fill="none" stroke="#0a2236" stroke-width="1.4" stroke-linejoin="round">
                            <path d="M4.5 16.5L5.8 7.8l4.2 3.6L12 6l2 5.4 4.2-3.6 1.3 8.7z" fill="#FFFFFF"/>
                            <path d="M5.5 19.5h13" stroke-width="2.2" stroke-linecap="round"/>
                        </svg>
                    </span>
                    <span class="a2zlv-kick">Established &amp; Complex</span>
                    <div class="a2zlv-name">
                        <h3>Strategic</h3>
                        <span class="a2zlv-badge">Premium</span>
                    </div>
    ...
    <section class="section-white" style="padding-top: 40px; padding-bottom: 80px;" id="reception-banner">
        <div class="container">
            <div class="reception-banner-wrapper">
                <img src="https://a2zaccounting.co.uk/wp-content/uploads/2025/08/Home-Page-photo.webp" alt="A2Z Accounting Office" class="reception-img" width="1200" height="500" loading="lazy" style="aspect-ratio: 12 / 5; object-fit: cover;">
                    ...
                    <div class="tax-content">
                        <div class="tax-card">
                            ...
                            <div class="tax-card-icon">
                                <img src="https://unpkg.com/lucide-static@0.400.0/icons/landmark.svg" alt="Strategy Icon" width="32" height="32" loading="lazy">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="tax-timeline-row tax-row-card">
                    ...
                    <div class="tax-content">
                        <div class="tax-card">
                            ...
                            <div class="tax-card-icon">
                                <img src="https://unpkg.com/lucide-static@0.400.0/icons/bar-chart-4.svg" alt="Compliance Icon" width="32" height="32" loading="lazy">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="container">
            ...
            <div class="sectors-grid">
                ...
                <div class="sector-card">
                    <img src="<?php echo esc_url($sector[3]); ?>" alt="<?php echo esc_attr($sector[0]); ?>" class="sector-img" width="400" height="300" loading="lazy" style="aspect-ratio: 4 / 3; object-fit: cover;">
    ...
    <section class="price-guarantee-section" id="price-guarantee">
        <div class="container">
            ...
            <a href="<?php echo esc_url(get_permalink()); ?>#a2z-feequote" class="pg-btn">GET A FEE ESTIMATE</a>
        </div>
    </section>
    ...
    <section class="switching-section" id="switching">
        <div class="container">
            ...
            <div class="switching-grid">
                ...
                <div class="switching-card">
                    <div class="sc-img-box">
                        <img src="https://res.cloudinary.com/dvvcwzp4n/image/upload/v1784193091/Discovery_Conversation_wkjcds.png" alt="Discovery Call" width="345" height="191" loading="lazy" style="aspect-ratio: 345 / 191; object-fit: cover;">
                    </div>
                    <div class="sc-body">
                ...
                <div class="switching-card">
                    <div class="sc-img-box">
                        <img src="https://res.cloudinary.com/dvvcwzp4n/image/upload/v1784193091/Structured_Handover_tittu0.png" alt="Handover" width="345" height="191" loading="lazy" style="aspect-ratio: 345 / 191; object-fit: cover;">
                    </div>
                    <div class="sc-body">
                        <div class="sc-header-row">
                ...
                <div class="switching-card">
                    <div class="sc-img-box">
                        <img src="https://res.cloudinary.com/dvvcwzp4n/image/upload/v1784193091/Ongoing_Management_omrii7.png" alt="Ongoing Management" width="345" height="191" loading="lazy" style="aspect-ratio: 345 / 191; object-fit: cover;">
                    </div>
                    <div class="sc-body">
                        <div class="sc-header-row">
            ...
            <div class="switching-footer-btn">
                <a href="<?php echo esc_url(get_permalink()); ?>#a2z-enquire" class="btn-switching">START THE CONVERSATION</a>
            </div>
        </div>
    </section>
                    ...
                    foreach ($latest_blogs as $post) :
                        ...
                        <div class="blog-card">
                            <div class="blog-img-box">
                                ...
                                <img src="<?php echo esc_url($img_url); ?>" alt="<?php echo esc_attr($post_title); ?>" width="400" height="250" loading="lazy" style="aspect-ratio: 1.6 / 1; object-fit: cover;">
                            </div>
                            <div class="blog-body">
                                <div class="blog-meta">
...
<script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Only preload the immediately next 2 slides — not all 14.
            // Loading all thumbnails at once wastes mobile bandwidth and
            // creates competing network requests that delay the LCP image.
            for (let i = 1; i <= 2; i++) {
                const img = new Image();
                img.src = heroTestimonials[i % heroTestimonials.length].thumb;
            }
            ...
            function renderCard(data) {
                ...
                if (thumbImg) {
                    ...
                    // Preload the slide after the one being rendered
                    const nextIdx = (currentHeroTestimonial + 1) % heroTestimonials.length;
                    if (nextIdx !== currentHeroTestimonial) {
                        const preloadImg = new Image();
                        preloadImg.src = heroTestimonials[nextIdx].thumb;
                    }
            ...
            // Seed the card content dynamically on slide change. No load seed needed (first card is hardcoded in HTML)
            ...
            function startHeroAutoplay() {
                ...
                heroAutoplayInterval = setInterval(() => {
                    if (!heroAnimating && !document.body.classList.contains('modal-open')) {
            ...
            // Pause on hover
            const heroCardEl = document.getElementById('heroTestimonialCard');
            if (heroCardEl) {
                heroCardEl.addEventListener('mouseenter', stopHeroAutoplay);
                heroCardEl.addEventListener('mouseleave', startHeroAutoplay);
            }

            // Pause when Loom modal is open
            const videoModalEl = document.getElementById('videoModal');
            if (videoModalEl) {
                videoModalEl.addEventListener('show.bs.modal', stopHeroAutoplay);
                videoModalEl.addEventListener('hidden.bs.modal', startHeroAutoplay);
            }
            ...
            window.updateHeroTestimonial = (direction = 'next') => {
                ...
                if (card) {
                    ...
                    // Set initial hidden state without triggering layout read
                    card.style.transition = 'none';
                    card.style.opacity    = '0';
                    card.style.transform  = `translateX(${inX})`;
                    // Use rAF instead of void offsetWidth so we don't force
                    // a synchronous layout recalculation on the main thread
            ...
            window.nextHeroTestimonial = () => {
                currentHeroTestimonial = (currentHeroTestimonial + 1) % heroTestimonials.length;
                updateHeroTestimonial('next');
                startHeroAutoplay(); // Reset interval on manual interaction
            };
...
<script>
// Open a2z CSS :target modal — bypasses all theme anchor interception
function a2zOpenModal(id) {
  history.replaceState(null, '', location.pathname + location.search);
  window.location.hash = id;
}
// Close a2z CSS :target modal
function a2zCloseModal() {
  window.location.hash = '!';
}
// CAPTURE-PHASE listener — runs before jQuery and ALL other handlers
// Forces close when clicking backdrop or × button, regardless of any interception
document.addEventListener('click', function(e) {
  var el = e.target;
  if (
    el.closest('.a2z-modal__backdrop') ||
    el.closest('.a2z-modal__close')
  ) {
    e.preventDefault();
    e.stopPropagation();
    a2zCloseModal();
...
document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') {
    var h = location.hash;
    if (h === '#a2z-enquire' || h === '#a2z-feequote') a2zCloseModal();
  }
});
</script>
