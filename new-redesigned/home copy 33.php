...
<script>
// Update #enquireModal title from the Continue button inside #a2z-enquire only.
// Uses a stored variable because relatedTarget is null after the a2z modal closes.
(function() {
  var _pendingEnquiryTitle = null;

  // Only capture from the Continue button that is inside the #a2z-enquire panel
  document.addEventListener('click', function(e) {
    var btn = e.target.closest('.a2z-eform__submit');
    if (!btn) return;
    // Only act when the button targets #enquireModal (not #gitModal)
    if (btn.getAttribute('data-bs-target') !== '#enquireModal') return;
    _pendingEnquiryTitle = btn.getAttribute('data-source') || null;
  }, true); // capture phase

  function applyModalSource() {
    var enquireModal = document.getElementById('enquireModal');
    if (!enquireModal) return;
    enquireModal.addEventListener('show.bs.modal', function() {
      if (!_pendingEnquiryTitle) return;
      var titleEl = enquireModal.querySelector('.modal-title') ||
                    enquireModal.querySelector('h5') ||
                    enquireModal.querySelector('h4') ||
                    enquireModal.querySelector('h3') ||
                    enquireModal.querySelector('h2');
      if (titleEl) titleEl.textContent = _pendingEnquiryTitle;
      _pendingEnquiryTitle = null; // reset after use
    });
  }
