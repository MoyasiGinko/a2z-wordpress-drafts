...
<script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Dynamically calculate and set min-height on .hero-card-info to prevent layout shifting
            function adjustHeroCardHeight() {
                const infoEl = document.querySelector('.hero-card-info');
                if (!infoEl) return;
                
                // Reset to measure naturally
                infoEl.style.minHeight = 'auto';
                
                const currentName = document.getElementById('heroAuthorName').innerHTML;
                const currentRole = document.getElementById('heroAuthorRole').innerHTML;
                const currentQuote = document.getElementById('heroQuote').innerHTML;
                
                let maxHeight = 0;
                
                // Measure each testimonial's rendered height
                heroTestimonials.forEach(t => {
                    document.getElementById('heroAuthorName').innerHTML = t.name;
                    document.getElementById('heroAuthorRole').innerHTML = t.role;
                    document.getElementById('heroQuote').innerHTML = t.quote;
                ...
                // Restore active card
                document.getElementById('heroAuthorName').innerHTML = currentName;
                document.getElementById('heroAuthorRole').innerHTML = currentRole;
                document.getElementById('heroQuote').innerHTML = currentQuote;
                
                if (maxHeight > 0) {
                    infoEl.style.minHeight = maxHeight + 'px';
                }
            }

            // Adjust size on DOM load, full page resource load, and window resize
            adjustHeroCardHeight();
            window.addEventListener('load', adjustHeroCardHeight);
            window.addEventListener('resize', adjustHeroCardHeight);
